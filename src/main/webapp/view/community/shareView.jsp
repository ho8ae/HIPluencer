							...
							<div class="card-body">
								<div class="row mb-2">
									<div class="col-md-8 d-flex align-items-center">
										<h1 class="h3 mb-2 fw-bold text-hipluncer">행사 공유 게시판</h1>
									</div>
									<div class="col-md-4">
										<div class="row">
											...
											<div class="col-md-12 mb-3">
												<div class="d-grid gap-2 d-md-flex justify-content-md-end">
													<button type="button" onclick="goShareForumWrite();"
			...
			<div id="board" class="container-board">
				...
				try {
					...
					if (post == null) {
						...
						sharePostDAO.increaseViews(postId); // Increase view count
				...
				<div class="card mb-3">
					...
					<div class="card-body">
						...
						<div id="map" style="width: 100%; height: 350px;"></div>
						 ...
						 <script>
        					// JSP에서 placeName과 address를 직접 JavaScript로 전달
       						 var placeName = '<%= placeName %>';
        						var address ='<%= address %>';

        							// 특수 문자가 포함될 수 있는 값을 이스케이프 처리
       							 placeName = placeName.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
       							 address = address.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");

      						  // 지도 초기화 함수 호출
     								   initializeMap(placeName, address);
    								</script>


					</div>
					<div class="d-flex card-footer">
						<form action="../../increaseShareLikes.jsp" method="post" >
    				...
    				<form action="../../editSharePost.jsp" method="get" class="ms-3">
      				  <input type="hidden" name="postId" value="<%=postId%>">
					...
					if (userID != null && userID.equals(post.getUserID())) {
					%>
					<form action="../../deleteSharePost.jsp" method="post"
						onsubmit="return confirm('정말로 이 글을 삭제하시겠습니까?');" class="ms-3">
						<input type="hidden" name="postId" value="<%=postId%>">
				...
				<form action="../../addCommentShare.jsp" method="post">
					<input type="hidden" name="postId" value="<%=postId%>">
					<div class="mb-3">
						<label for="commentContent" class="form-label">댓글 내용</label>
						<textarea class="form-control" id="commentContent" name="content"
							placeholder="무분별한 댓글은 제재사유가 될 수 있습니다." rows="3" required></textarea>
					</div>
						<div class="d-flex justify-content-end">
				...
				<div id="comments">
					<%
					List<ShareCommentDTO> comments = new ShareCommentDAO().getComments(postId);
					if (comments != null && !comments.isEmpty()) {
						for (ShareCommentDTO comment : comments) {
					%>
					<div class="card mb-2">
						<div class="card-body">
